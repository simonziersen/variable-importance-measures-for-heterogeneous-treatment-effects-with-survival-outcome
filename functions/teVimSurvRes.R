teVimSurvRes <- function(time,
  ...
  if(folds == 1){
    ...
    if(tauLearner == "S"){
      ...
      phi1 <- phi_tmp$phi1
      phi0 <- phi_tmp$phi0
      tau <- resSurv(S, newX = confounders, newtimes = evaltimes, newA = 1) -
      ...
      phi <- phi1 - phi0
      tau <- resSurv(S1, newX = confounders, newtimes = evaltimes, newA = 1) -
        resSurv(S0, newX = confounders, newtimes = evaltimes, newA = 0)
      taup <- mean(phi)
      taus_tmp <- do.call(pseudoReg, c(nuisance$pseudoReg, list(pseudo = tau, X = confounders[, sminus, drop = FALSE])))
      taus <- predict(taus_tmp, newX = confounders[, sminus, drop = FALSE])
    ...
    vals <- lapply(1:folds, function(i){
      ...
      if(tauLearner == "S"){
        ...
        phi0 <- phiSurvRes(surv = S0,
                        ...
                        predtimes = evaltimes)$phi$phi0
        phi <- phi1 - phi0
        tau_tmp <- resSurv(S1, newX = confounders, newtimes = evaltimes, newA = 1) -
          resSurv(S0, newX = confounders, newtimes = evaltimes, newA = 0)
        #taup <- mean(tau_tmp[k!=i])
        taup <- mean(phi[k!=i])
        tau <- tau_tmp[k == i]
...
teVimSurvRes_old <- function(time,
                         status,
                         treatment,
                         confounders,
                         s,
  ...
  if(folds == 1){
    ...
    if(!dr){
      tau <- resSurv(S, newX = confounders, newtimes = evaltimes, newA = 1) -
        resSurv(S, newX = confounders, newtimes = evaltimes, newA = 0)
      # tau_tmp <- predict(S, newX = confounders, newtimes = sort(time), newA = 1)$surv -
      #    predict(S, newX = confounders, newtimes = sort(time), newA = 0)$surv
      # tau2 <- Htu(tau_tmp, sort(time), evaltimes)[,1]
      taup <- mean(phi)
      taus_tmp <- do.call(pseudoReg, c(nuisance$pseudoReg, list(pseudo = tau, X = confounders[, sminus, drop = FALSE])))
      taus <- predict(taus_tmp, newX = confounders[, sminus, drop = FALSE])
    } else{
    ...
    vals <- lapply(1:folds, function(i){
      ...
      if(!dr){
        tau_tmp <- resSurv(S, newX = confounders, newtimes = evaltimes, newA = 1) -
          resSurv(S, newX = confounders, newtimes = evaltimes, newA = 0)
        # tau_tmp0 <- do.call(pseudoReg, c(nuisance$pseudoReg, list(pseudo = phi[k != i], X = confounders[k != i, , drop = FALSE])))
        # tau_tmp <- predict(tau_tmp0, newX = confounders)
        tau <- tau_tmp[k == i]
        taup <- mean(tau_tmp[k != i])
        #taup <- mean(phi[k != i])
        taus_tmp <- do.call(pseudoReg, c(nuisance$pseudoReg, list(pseudo = tau_tmp[k != i], X = confounders[k != i, sminus, drop = FALSE])))
        ...
        if(!drs){
          ...
          tau_tmp <- predict(tau_tmps, newX = confounders)
          tau <- tau_tmp$tau[k == i]
          taus_tmp <- do.call(pseudoReg, c(nuisance$pseudoReg, list(pseudo = tau_tmp$tau[k != i], X = confounders[k != i, sminus, drop = FALSE])))
          taus <- predict(taus_tmp, newX = confounders[k == i, sminus, drop = FALSE])
        } else{
          tau_tmps <- do.call(drSurv, list(time = time[k != i],
                                           status = status[k != i],
                                           ...
                                           transformation = "RMST",
                                           s = s))
          tau_tmp <- predict(tau_tmps, newX = confounders[k == i,])
          tau <- tau_tmp$tau
          taus <- tau_tmp$taus
        }
      }
      ...
      # var_thetas <- (1/N)*mean(eifThetas^2)
      # var_psis <- (1/N)*mean(eifPsis^2)
      vals <- list(varTau = varTau, varTaus = varTaus, thetas = thetas, psis = psis, taup = taup,
                   eifVarTau_uncen = eifVarTau_uncen, eifVarTaus_uncen = eifVarTaus_uncen, eifThetas_uncen = eifThetas_uncen, eifTaup_uncen = eifTaup_uncen,
                   nk = sum(k == i))
      vals
    })
    ...
    thetas <- sum(sapply(vals, function(x) x$nk/N * x$thetas))
    taup <- sum(sapply(vals, function(x) x$nk/N * x$taup))
    psis <- 1 - varTaus/varTau
    
    var_varTau <- sum(sapply(vals, function(x) x$nk/N * mean((x$eifVarTau_uncen - varTau)^2))) / N
    var_varTaus <- sum(sapply(vals, function(x) x$nk/N * mean((x$eifVarTaus_uncen - varTaus)^2))) / N
    var_thetas <- sum(sapply(vals, function(x) x$nk/N * mean((x$eifThetas_uncen - thetas)^2))) / N
    var_taup <- sum(sapply(vals, function(x) x$nk/N * mean((x$eifTaup_uncen - taup)^2))) / N
    var_psis <- sum(sapply(vals, function(x) x$nk/N * mean(((x$eifThetas_uncen - thetas - psis*(x$eifVarTau_uncen - varTau))/varTau)^2))) / N 
